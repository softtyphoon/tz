...
class firstp2p():
    ...
    def decision_maker(self, funds=None):
        ...
        print funds_last[2], funds[2]
        if funds_last[2] == funds[2]:
            print 'no change'
            return False
    ...
    def run(self):
        ...
        # for p in procs:
            # p.start()
        # 主循环
        pri_info = u'项目编号,类型,名称,子编号,总额,年化收益率,期限,方式,是否有优惠,优惠方式,优惠利率'.encode('gbk')
        pri_table = open('csv/main.csv', 'a+')
        pri_table.write(pri_info)
        pri_table.write('\n')
        last_p = list()
        times = 0
        while True:
                ...
                if go:
                    ...
                    while fund_done is False:     # 一直尝试，直到完成为止
                        ...
                        if result is False:
                            ...
                            # 重新登陆，获取新cookie
                            # print 'relogin . . .'
                            if self.f_header == _header:
                                self.f_header = _query_header
                            else:
                                self.f_header = _header
                            time.sleep(3)
                            continue
                        else:
                            print 'success: %s' % (f[0])
